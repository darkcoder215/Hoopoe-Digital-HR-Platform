...
export default function SettingsPage() {
  ...
  return (
    <>
      <Topbar title="الإعدادات" subtitle="إعدادات المنصة" />
      <div className="p-8 max-w-3xl mx-auto space-y-6">
        ...
        <Card className="card-entrance">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              <h2 className="text-sm font-black text-hoopoe-black">تحليل الذكاء الاصطناعي</h2>
              <p className="text-xs text-hoopoe-black/40 font-bold">تكوين سلوك الفرز الآلي</p>
            </div>
          </div>
          <div className="space-y-4">
            <div className="flex items-center justify-between p-3 rounded-xl bg-hoopoe-surface/50">
              <div>
                <p className="text-xs font-bold text-hoopoe-black">التحليل التلقائي عند الرفع</p>
                <p className="text-[10px] text-hoopoe-black/40 font-bold">تشغيل تحليل الذكاء الاصطناعي تلقائياً عند رفع سيرة ذاتية</p>
              </div>
            ...
            <div className="p-3 rounded-xl bg-hoopoe-surface/50">
              <div className="flex items-center justify-between mb-2">
                <div>
                  <p className="text-xs font-bold text-hoopoe-black">الحد الأدنى للتقييم</p>
                  <p className="text-[10px] text-hoopoe-black/40 font-bold">المرشحون أقل من هذا التقييم سيتم تمييزهم للمراجعة</p>
                </div>
              ...
              <div className="flex justify-between text-[9px] text-hoopoe-black/30 mt-1 font-bold">
                <span>٠ (قبول الجميع)</span>
                <span>١٠٠ (صارم جداً)</span>
              </div>
            </div>
          </div>
        </Card>
        ...
        <Card className="card-entrance stagger-2">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              ...
              <p className="text-xs text-hoopoe-black/40 font-bold">مراحل خط التوظيف النشطة</p>
            </div>
          </div>
        ...
        <Card className="card-entrance stagger-3">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              <h2 className="text-sm font-black text-hoopoe-black">الإشعارات</h2>
              <p className="text-xs text-hoopoe-black/40 font-bold">إدارة تفضيلات التنبيهات</p>
            </div>
          </div>
          <div className="space-y-3">
            {[
              { label: 'إشعارات البريد الإلكتروني', desc: 'استقبال تنبيهات البريد الإلكتروني للأحداث المهمة', enabled: emailNotifs, toggle: () => setEmailNotifs(!emailNotifs) },
              { label: 'تنبيهات تغيير المرحلة', desc: 'إشعار عند انتقال المرشحين بين المراحل', enabled: stageNotifs, toggle: () => setStageNotifs(!stageNotifs) },
              { label: 'اكتمال تقرير الذكاء الاصطناعي', desc: 'تنبيه عند جاهزية تحليل الذكاء الاصطناعي', enabled: reportNotifs, toggle: () => setReportNotifs(!reportNotifs) },
            ].map((n) => (
        ...
        <Card className="card-entrance stagger-4">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              <h2 className="text-sm font-black text-hoopoe-black">أعضاء الفريق</h2>
              <p className="text-xs text-hoopoe-black/40 font-bold">الأشخاص الذين لديهم صلاحية الوصول</p>
            </div>
          </div>
          <div className="space-y-2">
            {[
              { name: 'محمد ي. عمارة', role: 'المدير التنفيذي', email: 'mohammad@hoopoe.digital' },
              { name: 'مدير الموارد البشرية', role: 'مسؤول', email: 'hr@hoopoe.digital' },
              { name: 'قائد التقنية', role: 'مراجع', email: 'tech@hoopoe.digital' },
            ].map((m) => (
        ...
        <Card className="card-entrance stagger-5">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h2 className="text-sm font-black text-hoopoe-black">الهوية البصرية</h2>
              <p className="text-xs text-hoopoe-black/40 font-bold">ألوان علامة هوبو ديجيتال التجارية</p>
            </div>
          </div>
          <div className="flex gap-2">
            {[
              { name: 'أسود', hex: '#20242C' },
              { name: 'برتقالي', hex: '#CE8345' },
              { name: 'بني', hex: '#A34823' },
              { name: 'برتقالي فاتح', hex: '#E7A15E' },
              { name: 'ذهبي', hex: '#F9CF9D' },
              { name: 'كحلي', hex: '#252A35' },
            ].map((c) => (
              <div key={c.hex} className="flex-1 text-center group">
                <div className="w-full aspect-square rounded-xl mb-1.5 transition-transform duration-200 group-hover:scale-105" style={{ backgroundColor: c.hex }} />
