...
import {
  ...
  ChevronRight, GripVertical, Search,
} from 'lucide-react';
...
function CandidateCard({ candidate, onMove }: { candidate: Candidate; onMove: (id: string, stage: PipelineStage) => void }) {
  ...
  return (
    <div className="group bg-white rounded-lg border border-hoopoe-lt-gray p-3 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 animate-scale-in">
      <div className="flex items-start gap-2.5">
        <div className="w-8 h-8 rounded-full bg-hoopoe-orange/10 text-hoopoe-orange flex items-center justify-center text-[10px] font-bold flex-shrink-0">
          {getInitials(candidate.name)}
        </div>
        <div className="flex-1 min-w-0">
          <Link href={`/candidates/${candidate.id}`} className="text-xs font-semibold text-hoopoe-black hover:text-hoopoe-orange transition-colors block truncate">
            {candidate.name}
          </Link>
          <p className="text-[10px] text-hoopoe-black/40 truncate">{candidate.position}</p>
        </div>
        {candidate.aiReport && (
      ...
      <div className="mt-2.5 flex items-center justify-between">
        <span className="text-[9px] text-hoopoe-black/30">{formatRelativeTime(candidate.uploadedAt)}</span>
        {nextStage && (
          <button
            onClick={(e) => { e.stopPropagation(); onMove(candidate.id, nextStage.key); }}
            className="opacity-0 group-hover:opacity-100 flex items-center gap-0.5 text-[9px] font-medium text-hoopoe-orange hover:text-hoopoe-brown transition-all cursor-pointer"
          >
  ...
  return (
    <div className="flex flex-col min-w-[240px] max-w-[280px] animate-fade-in">
      ...
      <div className="flex items-center gap-2 mb-3 px-1">
        ...
        <span className="text-xs font-semibold text-hoopoe-black truncate">{stage.label}</span>
        <span className="ml-auto text-[10px] font-bold text-hoopoe-black/30 bg-hoopoe-surface px-1.5 py-0.5 rounded-md">
          {candidates.length}
        </span>
      </div>
      ...
      <div className="flex-1 space-y-2 p-1 min-h-[200px] rounded-xl bg-hoopoe-surface/40 border border-dashed border-hoopoe-lt-gray/50">
        {candidates.length === 0 ? (
          <div className="flex items-center justify-center h-full text-[10px] text-hoopoe-black/20 italic py-12">
            No candidates
          </div>
        ) : (
...
export default function PipelinePage() {
  ...
  return (
    <>
      <Topbar title="Pipeline Board" subtitle="Drag candidates through recruitment stages" />
      <div className="p-6">
        {/* Search bar */}
        <div className="flex items-center gap-3 mb-6 animate-fade-in">
          <div className="relative flex-1 max-w-xs">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 text-xs rounded-lg border border-hoopoe-lt-gray bg-white focus:border-hoopoe-orange focus:ring-2 focus:ring-hoopoe-focus outline-none w-full transition-all"
            />
          </div>
          <div className="flex items-center gap-1 text-[10px] text-hoopoe-black/40">
            <GripVertical size={12} />
            Hover a card and click the arrow to advance stages
          </div>
        </div>
