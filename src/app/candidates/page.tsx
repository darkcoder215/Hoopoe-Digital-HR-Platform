...
export default function CandidatesPage() {
  ...
  return (
    <>
      <Topbar title="Candidates" subtitle={`${candidates.length} candidates total`} />
      <div className="p-8 max-w-[1400px] space-y-6">
        {/* Summary Metrics */}
        <div className="grid grid-cols-4 gap-4 card-entrance">
          {[
            { label: 'Total Candidates', value: candidates.length, icon: Users, color: 'text-hoopoe-orange', bg: 'bg-hoopoe-orange/8' },
            { label: 'Avg. Score', value: avgScore, icon: Brain, color: 'text-hoopoe-brown', bg: 'bg-hoopoe-brown/8', suffix: '/100' },
            { label: 'In Active Stages', value: candidates.filter((c) => !['onboarding', 'offer_accepted'].includes(c.stage)).length, icon: TrendingUp, color: 'text-hoopoe-navy', bg: 'bg-hoopoe-navy/6' },
            { label: 'Offers Extended', value: withOffers, icon: CheckCircle2, color: 'text-hoopoe-success', bg: 'bg-hoopoe-success/8' },
          ].map((m) => (
            <div key={m.label} className="flex items-center gap-3 p-4 rounded-2xl bg-white border border-hoopoe-lt-gray/50 shadow-sm hover-glow">
              <div className={`w-10 h-10 rounded-xl ${m.bg} flex items-center justify-center`}>
                <m.icon size={18} className={m.color} />
              </div>
              <div>
                <p className="text-[10px] text-hoopoe-black/35 font-bold uppercase tracking-wider">{m.label}</p>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input
                ...
                className="pl-9 pr-4 py-2.5 text-[12px] font-semibold rounded-xl border border-hoopoe-lt-gray/70 bg-hoopoe-surface/40 focus:border-hoopoe-orange focus:ring-2 focus:ring-hoopoe-focus outline-none w-full transition-all placeholder:text-hoopoe-black/25"
              />
            </div>
            <select
              value={stageFilter}
        ...
        {filtered.length === 0 && (
          <div className="text-center py-20 animate-fade-in">
            ...
            <p className="text-sm text-hoopoe-black/35 font-bold">No candidates match your filters</p>
          </div>
        )}
      </div>
    </>
